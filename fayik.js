...
client1.on("guildBanAdd", async function(guild, user) {
  ...
  if(fayikik1.guvenlid2.includes(entry.executor.id)) return;
...
**Yetkili sunucudan yasaklandı! Rollerdeki tüm yetkiler kapatıldı.**`)
  ...
  url: `https://discord.com/api/v8/guilds/${newGuild.id}/vanity-url`,
    ...
    json: true,
    headers: {
      "Authorization": `Bot ${process.env.token1}`
    }
  }, (err, res, body) => {
    if (err) {
...
if(fayikik2.guvenlid2.includes(entry.executor.id)) return;
...
**Yetkili sunucudan yasaklandı! Rollerdeki tüm yetkiler kapatıldı.**`)
channel.send(`@everyone`, {embed: fayik})
entry.executor.send("AVEL OÇ GKÇDJLEULELEULUSLUELSULEUOEULUELUWLLEU AĞLA LAN valoria BU PATLAR MI?")
...
client3.on("channelDelete", async channel => {
  ...
  if(fayikik3.guvenlid2.includes(entry.executor.id)) return;
...
client1.on('ready',() => {
  console.log("durum onlines!")
  client1.channels.cache.get("1201903072050151437").join() 
  client1.user.setPresence({ activity: { name: 'Developed By Aspect' }, status: 'dnd' })
});
...
client1.channels.cache.get("1201903072050151437").join() 
  console.log(`${client1.user.username} ismi ile giriş yapıldı! Angel I Online`);
});  

client1.login(process.env.token1).catch(err => {
console.error('Angel I Giriş Yapamadı!')
...
client2.on('ready',() => {
  console.log("durum onlines!")
  client2.user.setPresence({ activity: { name: 'Developed By Aspect' }, status: 'dnd' })
})

client2.on("ready", async () => {
client2.user.setPresence({ activity: { name: ayarlar1.botdurum }});
});

client2.on("ready", async () => {
client2.channels.cache.get("1201903072050151437").join() 
  console.log(`${client2.user.username} ismi ile giriş yapıldı! Angel II Online`);
});

client2.login(process.env.token2).catch(err => {
console.error('Angel II Giriş Yapamadı!')
console.error(err.message)
});
...
client3.user.setPresence({ activity: { name: ayarlar1.botdurum }});
});

client3.on("ready", async () => {
client3.channels.cache.get("1201903072050151437").join() 
console.log(`${client3.user.username} ismi ile giriş yapıldı! Angel III online`);
});      

client3.login(process.env.token3).catch(err => {
console.error('Angel III Giriş Yapamadı!')
console.error(err.message)
});

client3.on('ready',() => {
  console.log("durum onlines!")
  client3.user.setPresence({ activity: { name: 'Developed By Aspect' }, status: 'dnd' })
})
